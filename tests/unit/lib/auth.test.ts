...
describe("checkIsAdmin", () => {
  ...
  it("should return true when user is admin", async () => {
    const mockQueryBuilder = {
      ...
      eq: vi.fn().mockReturnThis(),
      maybeSingle: vi.fn().mockResolvedValue({
        data: { id: "1", email: "admin@test.com", role: "admin", is_club_member: true },
        error: null,
      }),
    };
  ...
  it("should return false when user is not admin", async () => {
    const mockQueryBuilder = {
      ...
      eq: vi.fn().mockReturnThis(),
      maybeSingle: vi.fn().mockResolvedValue({
        data: { id: "1", email: "viewer@test.com", role: "viewer", is_club_member: true },
        error: null,
      }),
    };
    ...
    const { checkIsAdmin } = await import("@/lib/auth");
    const result = await checkIsAdmin("viewer@test.com");

    expect(result).toBe(false);
  });

  it("should return false when no email provided and no current user", async () => {
    mockSupabase.auth.getSession.mockResolvedValue({
      data: { session: null },
      error: null,
    });
  ...
  it("should get email from current user when not provided", async () => {
    mockSupabase.auth.getSession.mockResolvedValue({
      data: {
        session: {
          user: {
